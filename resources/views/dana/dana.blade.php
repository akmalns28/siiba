...
@section('container')
        ...
        <div class="card-body">
            <div class="row align-items-center justify-content-between mb-4 ">
                <div class="col-sm-6 col-md-5 ">
                    <form action="{{ route('dana') }}" method="GET" class="d-flex">
                ...
                <div class="col-sm-6 col-md-auto">
                    <a href="{{ route('tambah-dana') }}" class="text-white " style="text-decoration: none;">
            ...
            <div class="table-responsive text-wrap">
                <table class="table table-borderless">
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach ($data as $index => $dana)
                                <tr>
                                    <th scope="row">{{ $index + $data->firstItem() }}</th>
                                    <td>{{ $dana->dana }}</td>
                                    <td>
                                        <div class="button">
                                            <a href="{{ route('tampil-dana', $dana->id) }}"
                                            ...
                                            <a href="#" class="btn btn-icon btn-sm btn-primary dlt-dana"
                                                data-id="{{ $dana->id }}" data-name="{{ $dana->dana }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 24 24">
                                                    <path fill="currentColor"
    ...
    @push('js')
        ...
        <script>
            $('.dlt-dana').click(function() {
                var iddana = $(this).attr('data-id');
                var dana = $(this).attr('data-name');
                Swal.fire({
                    title: 'Apakah Anda Yakin??',
                    text: "Kamu akan menghapus data " + dana + "",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        window.location = "/delete-dana/" + iddana + ""
                        Swal.fire(
                            'Deleted!',
                            'Data ' + dana + ' Telah Dihapus',
                            'success'
                        )
                    }
                });
