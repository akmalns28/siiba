...
@section('container')
        ...
        <div class="card-body">
            <div class="row align-items-center justify-content-between mb-4 ">
                <div class="col-sm-6 col-md-5 ">
                    <form action="{{ route('sub-kategori') }}" method="GET" class="d-flex">
                ...
                <div class="col-sm-6 col-md-auto">
                    <a href="{{ route('tambah-sub-kategori') }}" class="text-white " style="text-decoration: none;">
            ...
            <div class="table-responsive text-wrap">
                <table class="table table-borderless">
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach ($data as $index => $sub_kategori)
                                <tr>
                                    <th scope="row">{{ $index + $data->firstItem() }}</th>
                                    <td>{{ $sub_kategori->kategori->kategori }}</td>
                                    <td>{{ $sub_kategori->kode_sub_kategori }}</td>
                                    <td>{{ $sub_kategori->sub_kategori }}</td>
                                    <td>
                                        <div class="button">
                                            <a href="{{ route('tampil-sub-kategori', $sub_kategori->id) }}"
                                            ...
                                            <a href="#" class="btn btn-icon btn-sm btn-primary dlt-sub_kategori"
                                                data-id="{{ $sub_kategori->id }}"
                                                data-name="{{ $sub_kategori->sub_kategori }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 24 24">
                                                    <path fill="currentColor"
    ...
    @push('js')
        ...
        <script>
            $('.dlt-sub_kategori').click(function() {
                var idsub_kategori = $(this).attr('data-id');
                var sub_kategori = $(this).attr('data-name');
                Swal.fire({
                    title: 'Apakah Anda Yakin??',
                    text: "Kamu akan menghapus data " + sub_kategori + "",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        window.location = "/delete-sub_kategori/" + idsub_kategori + ""
                        Swal.fire(
                            'Deleted!',
                            'Data ' + sub_kategori + ' Telah Dihapus',
                            'success'
                        )
                    }
                });
